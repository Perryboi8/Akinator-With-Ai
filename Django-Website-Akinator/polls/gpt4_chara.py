...
def get_initial_question():
    response = openai.ChatCompletion.create(
        model=MODEL,
        messages=[
            {"role": "system", "content": "You are trying to guess a character based on the user's answers to yes or no questions."},
...
def get_next_question(log, previous_questions):
    try:
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {"role": "system", "content": (
                    "You are a character-guessing assistant. Based on the user's answers, "
...
def evaluate_genre_confidence(log):
    try:
        response = openai.ChatCompletion.create(
            model=MODEL,
            messages=[
                {"role": "system", "content": "You are evaluating the confidence level for identifying the character genre and background information based on the user's answers."},
...
def identify_genre(log):
    try:
        response = openai.ChatCompletion.create(
            model=MODEL,
            messages=[
                {"role": "system", "content": "Identify the genre and background information based on the user's answers provided in the log."},
                {"role": "user", "content": log},
                {"role": "user", "content": "Provide the genre or setting as a concise description."}
            ],
            temperature=0.5,
        )
...
def make_final_guess(log):
    response = openai.ChatCompletion.create(
        model=MODEL,
        messages=[
            {"role": "system", "content": "Based on the log of answers, make your final guess of character name."},
            {"role": "user", "content": log},
            {"role": "user", "content": "Provide only the character name of the final guess."}
        ],
        temperature=0.7,
